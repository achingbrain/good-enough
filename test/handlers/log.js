...
test.cb('handlers/log should transform log event', (t) => {
  const event = {
...
test.cb('handlers/log should transform log event where data is an Error', (t) => {
  const event = {
    request: faker.random.uuid(),
    tags: [LEVELS.INFO, faker.lorem.words(1)],
    timestamp: new Date(),
...
test.cb('handlers/log should transform log event where data is an Error with no stack trace', (t) => {
  const event = {
    request: faker.random.uuid(),
    tags: [LEVELS.INFO, faker.lorem.words(1)],
    timestamp: new Date(),
